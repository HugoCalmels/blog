...
const Carrousel = (props) => {
  ...
  const moveCarrouselToImage = (newIndex) => {
    carrouselFrameElem.current.style.transform = `translateX(-${
      newIndex * defaultImageWidth
    }px)`;
  ...
  const actionsInterval = () => {
    intervalRef.current = setInterval(() => {
      if (
        window.location.pathname === "/gaelle-boucherit" &&
        carrouselArray.length !== 0
      ) {
        setTimer((timer) => timer + 1);
        lastIndex = currentIndexInterval;

        if (
          currentIndexInterval < carrouselArray.homes.length - 1 &&
          carrouselArray.homes.length - 1 !== null
        ) {
        ...
        if (carrouselFrameElem.current) {
          carrouselFrameElem.current.style.transform = `translateX(-${
            currentIndexInterval * defaultImageWidth
          }px)`;
        }
      }
    }, 5000);
  ...
  return (
    <div className="b-index-carrousel-wrapper">
      {isAuthCookie ? (
        <div
          className="b-index-carrousel-edit-modal-toggle-btn active"
          onClick={(e) => openEditCarrouselModal(e)}
          ref={bntOpenEditModalRef}
        >
      ...
      <EditCarrousel
        ...
        carrouselArray={carrouselArray}
        editCarrouselRef={editCarrouselRef}
        closeEditModal={closeEditModal}
      />

      <div
        className="b-index-carrousel-images-container"
        ref={carrouselFrameElem}
      >
        {carrouselArray.homes &&
          carrouselArray.homes.map((image, index) => (
            <>
              {index === 0 ? (
                <div
                  className="b-index-carrousel__img_unit"
                  style={{ left: 0 + "px" }}
                  ref={addImageToRef}
                >
      ...
      <div className="b-index-carrousel-btns-container">
        {pause ? (
          <div
            className="b-index-carrousel-btn-play"
            onClick={togglePause}
          ></div>
        ) : (
          <div
            className="b-index-carrousel-btn-pause"
            onClick={togglePause}
          ...
          >
            ...
            <div
              className="b-index-cbp-bar"
              ref={carrouselPauseElemRight}
            ></div>
          </div>
        )}

        {carrouselArray.homes &&
          carrouselArray.homes.map((el, index) => (
            <div
              className="b-index-carrousel__img_unit__btn"
