...
const EditPerformance = (props) => {
  ...
  const editPerformanceAPI = async (file, id) => {
    ...
    const response = await fetch(
      `${BASE_URL}/api/v1/performances/${id}`,
      config
    );
    const data = await response.json();
  ...
  return (
    <div
      className="bp-perf-edit-perf-modal-wrapper"
      ref={props.editPerformanceModalRef}
    >
      <h4 id="bp-perf-edit-perf-modal-title">
        Editer : {props.selectedValue.title}
      </h4>
