...
const TopBar = (props) => {
  const handleChange = (e) => {
    ...
    const foundPerf = props.value.find(
      (perf) => perf.id == e.target.selectedOptions[0].id
    );
    ...
    <div className="bp-topbar" ref={props.topBarElem}>
      <div className="bp-topbar-container">
        <div className="bp-topbar-entries">
          {props.value &&
            props.value.map((performance) => (
              <>
                {performance.featuring.length > 0 ? (
                  <>
                    <div
                      className="bp-topbar-perf"
                      onClick={() => props.setSelectedValue(performance)}
                    >
                  ...
                  <div className="bp-topbar-perf no-feat">
                    <h5 onClick={() => props.setSelectedValue(performance)}>
                      {performance.title}
                    </h5>
                  </div>
                )}
      ...
      <div className="bd-top-bar-categories-mobile-perf">
        <select
          id="b-select-mobile-perf-mobile"
