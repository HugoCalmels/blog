...
const Subscribers = () => {
  ...
  const tryToDestroyGuest = (e, guest) => {
    ...
    let answer = window.confirm(`voulez vous supprimer "${guest.email}" ?`);
    if (answer) {
          ...
          <table className="admin-subs-table">
            ...
            {guests.map((guest, index) => (
              <tr>
                ...
                <td className="b-td-joindate">
                  {generateFormatedDate(guest.created_at)}
                </td>
                <td className="b-td-destroy-guest">
                  <span>
                    <FaTrashAlt
                      style={btnStyle}
                      onClick={(e) => tryToDestroyGuest(e, guest)}
                    />
                  </span>
                </td>
              </tr>
