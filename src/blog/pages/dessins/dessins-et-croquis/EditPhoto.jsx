...
const EditPhoto = (props) => {
  ...
  const mainModalElem = useRef(null)
  const submitEditImageToAPI = async (newImage, cateID) => {
    ...
    // 2 différentes manip, selon si 
    // - il faut changer de categorie => completed
    // - on reste dans la mm caté ( le placement doit rester le mm )
    ...
    if (props.editSelectedCategory.id !== datafetched.dessin_category_id) {
      ...
      const removedImage = value.map((category) => {
        return {
          ...category, dessins: category.dessins.filter((image) => {
      ...
      let test = value.map((category) => {
        return {
          ...category, dessins: category.dessins.map((img) => {
    ...
    mainModalElem.current.style.display = "none"
    setValue(newValue)
    //window.location.reload(false);
  };
  const tryToEditImage = (e) => {
    e.preventDefault();
    let getSelects = document.querySelectorAll(".bdl-custom-select-edit");
    console.log("?????")
    console.log("?????")
 
    console.log(categorySelected)

    //const categorySelectedID = categorySelected.target.options[categorySelected.selectedIndex].dataset.id
   // const categorySelectedName = categorySelected.target.options[categorySelected.selectedIndex].dataset.value
    ...
    if (categorySelected === null) {
      ...
      cateID = categorySelected.target.options[categorySelected.target.selectedIndex].dataset.id
    }
    ...
    if (e.target[0].files[0] !== undefined) {
      ...
      console.log("TEST WITHOUT IMAGE")
      console.log('+++++++++++++++++++++')
      console.log('+++++++++++++++++++++')
      console.log('+++++++++++++++++++++')
  ...
  useEffect(() => {
    console.log(".................")
    console.log(".................")
    console.log(props.editSelectedImage)
    console.log(".................")
    console.log(".................")
  ...
  return (
    <div className="bd-edit-photo" ref={mainModalElem}>
      ...
      <form className="bd-edit-form" onSubmit={(e) => tryToEditImage(e)} ref={editFormElem}>
        
        <div className="bd-edit-input-container-upload">
        <label htmlFor="bd-edit-image">1. Upload nouvelle image ( optionnel )</label>
        ...
        <select id="select-categories-edit" onChange={(e)=>setCategorySelected(e)}>
